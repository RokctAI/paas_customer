...
// Ported from commerce marketplace_sdk's WalletSendScreen (the profile
// wallet card's Send sheet) as part of re-homing the wallet card into
// wallet_sdk. The donor already read only base_sdk symbols and drove
// base_sdk's WalletRepositoryFacade (confirmRecipient / sendWalletToPhone /
// sendWalletByCode — implemented by this SDK's WalletRepository), so the
// port is verbatim beyond this note.
  ...
  Widget _buildConfirmedRecipient() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(
            backgroundColor: AppStyle.primary.withValues(alpha: 0.1),
            radius: 20.r,
            child: Text(
