...
class PaymentsRepository implements PaymentsRepositoryFacade {
  ...
  Future<ApiResult<PaymentsResponse>> getPayments() async {
    try {
      ...
      final response = await client.get(
        '/api/v1/method/paas.api.payment.get_payment_gateways',
      );
  ...
  Future<ApiResult<TransactionsResponse>> createTransaction({
    ...
    try {
      ...
      final response = await client.post(
        '/api/v1/method/paas.api.payment.create_transaction',
  ...
  Future<ApiResult<List<SavedCardModel>>> getSavedCards() async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/method/paas.api.payment.get_saved_cards',
      );
  ...
  Future<ApiResult<String>> tokenizeCard({
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.post(
        '/api/v1/method/paas.api.payment.tokenize_card',
        data: {
  ...
  Future<ApiResult<bool>> deleteCard(String cardId) async {
    try {
      final client = dioHttp.client(requireAuth: true);
      await client.post(
        '/api/v1/method/paas.api.payment.delete_card',
  ...
  Future<ApiResult<String>> processTokenPayment(
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      await client.post(
        '/api/v1/method/paas.api.payment.process_token_payment',
  ...
  Future<ApiResult<String>> processDirectCardPayment(
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.post(
        '/api/v1/method/paas.api.payment.process_direct_card_payment',
        data: {
