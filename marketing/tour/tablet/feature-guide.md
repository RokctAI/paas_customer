# Juvo — Feature Guide

The multi-store marketplace

> This guide is generated automatically on every merge: CI builds the
> app, walks the guided tour on an Android emulator, and captures
> each screen below.

## 1. Welcome to Juvo

![Welcome to Juvo](screenshots/01-welcome.png)

Juvo is every store in one app - sign in to get started.

## 2. Sign in

![Sign in](screenshots/02-auth_login.png)

Signing in to Juvo is quick and simple.

## 3. Create an account

![Create an account](screenshots/03-auth_register.png)

New to Juvo? Create your account in a few easy steps.

## 4. Reset your password

![Reset your password](screenshots/04-auth_reset_password.png)

Forgot your password? Juvo gets you back in with a secure reset.

## 5. Every shop, one home

![Every shop, one home](screenshots/05-marketplace_home.png)

Deals, categories and every local shop on one screen.

## 6. Shop by category

![Shop by category](screenshots/06-marketplace_category.png)

Pick a category and see just the shops that stock it.

## 7. Find it fast

![Find it fast](screenshots/07-marketplace_search.png)

One search across shops and products, with filters a tap away.

## 8. Your favourites, kept

![Your favourites, kept](screenshots/08-marketplace_favourites.png)

Heart a shop once and it waits for you here - your favourites, one tap away.
